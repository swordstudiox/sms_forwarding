...
- WiFi 配网：支持最多 8 个已知 WiFi 网络，启动/重连时扫描当前环境并优先连接信号最好的已知网络；正常连接后可通过 IP 或 `sms.local` 访问。
- 账号密码：Web 管理账号可单独保存。
- 时间设置：NTP 服务器和时区可配置。
...
- 支持 SoftAP 配网、已知 WiFi 列表扫描选优接入、`sms.local` mDNS、NTP 时间同步、AT 终端和运行状态查看。
...
首次启动时设备会读取已知 WiFi 网络列表（最多 8 个），先扫描当前环境，匹配已保存 SSID，并优先连接当前 RSSI 最好的已知网络。
...
连接热点后访问 `http://192.168.1.1` 配网；设备连上路由器后，可通过串口日志里的 IP 地址或 `http://sms.local` 打开 Web UI。
