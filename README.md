...
> ⚠️ **测试版固件**：本分支为重构 / 优化的新方案，仍在持续验证中，请先小范围试用、确认稳定后再长期值守。其中**「保号定时任务」（蜂窝 HTTP payload 下载 / 发短信 / USSD）为实验性功能**——不同运营商、不同卡对“动账 / 保号”的判定差异很大，**请务必自行确认保号是否真的生效**（如查看运营商 App 的用量或最后活跃日期）。遇到任何问题（收不到短信、转发 / 推送失败、保号无效、崩溃重启等），欢迎及时[提交 issue](https://github.com/MineSunshineone/sms_forwarding/issues)。
...
- **自动保号（绝对日期，断电不忘）｜🧪 实验性**：按设定周期（如 giffgaff 每 180 天）自动维持 SIM 活跃，基准日持久化于 NVS，断电重启对时后仍准确。动作可选 **蜂窝 HTTP payload 下载（默认 `http://gg.incrafttime.top/api/payload?size=64342`，约 63KB，可换 URL）/ 发短信 / USSD 查询**。⚠️ 各运营商动账判定不同，请自行确认保号确实生效。
...
- **配网友好**：未配置 / 连不上 WiFi 时自动开启配网热点；运行中长按 BOOT 5 秒也可强制开启。热点名为 `SMS-Forwarder-XXXX`（后四位随机），固定访问 `http://192.168.1.1` 配置新 WiFi。
...
> **模组兼容性**：整个 **ML307 系列**（ML307R / ML307C / ML307A 及各 “DC” 变体）共用同一套中移 AT 命令集，PDU 短信 / CNMI / CMGL / CMGS / CEREG / CUSD / MHTTP 等均通用，可直接替换。请用模组的 **UART 焊盘**（不是 USB 口）连接，并把 EN/电源脚接到 GPIO5 以支持硬件断电恢复。
...
3. 配网热点：设备连不上旧 WiFi 时会自动广播 `SMS-Forwarder-XXXX`（后四位随机）；也可在固件运行中**长按 BOOT 5 秒**强制开启。手机/电脑连接该热点后访问固定地址 **`http://192.168.1.1`**，登录网页后到「WiFi 设置」保存新网络。
...
1. 上电后如果连上路由器，浏览器访问设备 IP（或 `http://sms.local`）。
2. 如果到了新地方、旧 WiFi 不存在，等待约 20 秒后连接设备热点 `SMS-Forwarder-XXXX`，打开 **`http://192.168.1.1`** 配置新 WiFi；设备运行中也可以长按 BOOT 5 秒手动打开该热点。
3. 默认账号密码 **`admin` / `admin123`**，**请立即在“系统设置”里改掉**。
4. 在“转发通道”配好邮件 / 推送，可点“测试推送”当场验证；按需在“定时任务”里开保号。
